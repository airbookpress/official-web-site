...
function _the_logo() {
    ...
    echo '<' . $tag . ' class="logo"><a href="' . get_bloginfo('url') . '" title="' . get_bloginfo('name') . ( get_bloginfo('description')?_get_delimiter() . get_bloginfo('description'):'') . '"><img src="'.$src.'"><span>' . get_bloginfo('name') . '</span></a></' . $tag . '>';
}
...
function _the_ads($name='', $class=''){
    ...
    if( _hui('hide_for_logger') && is_user_logged_in() ) 
        return;

    if( strstr($name, 'pager') ){
        global $pages;
 ...
 * focusbox
 */
function _the_focusbox($title_tag='h1', $title='', $text='', $counts=''){
    if( $title ){
        if( !$title_tag ){
            $title_tag = 'h1';
        }
    ...
    if( $counts ){
        $counts = '<div class="focusbox-counts">'.$counts.'</div>';
    }

    echo '<div class="focusbox"><div class="container">'.$title.$text.$counts.'</div></div>';
}
...
function _bodyclass() {
    ...
    if ( (is_single() || is_page()) && comments_open() && !_hui('kill_comment_s') ) {
        $class .= ' comment-open';
    }
    ...
    if( _hui('phone_list') ){
        $class .= ' mlist-'._hui('phone_list');
    }

    if( _hui('nav_fixed_top') ){
        $class .= ' nav-fixed-top';
    }
    ...
    if( _hui('zoom_image') && wp_is_mobile() ){
        $class .= ' m-zoomimage';
    }
        ...
        foreach($images as $src){
            if( _hui('thumb_postfirstimg_lastname') ){
                if( _hui('thumb_postfirstimg_lastname_next') ){
                    $src .= _hui('thumb_postfirstimg_lastname');
                }else{
...
function _load_scripts() {
    if (!is_admin()) {
        ...
        wp_register_script( 'jquery', get_stylesheet_directory_uri().'/js/jquery.js', false, _the_theme_version(), false );
...
function _head_css() {
    ...
    if( $skc && $skc !== '#FF6651' ){
        $styles .= "a:hover{color:{$skc}}.cat-menus .container>ul>li.current-cat.catitem-parent>a::before,.cat-menus .container>ul>li.catitem-parent>a:hover::before,.cat-menus .container>ul>li.current-cat-parent>a::before, .cat-menus .container>ul>li.current-cat-ancestor>a::before{border-top-color:{$skc}}.cat-menus .container>ul>li.current-cat>a,.cat-menus .container>ul>li.current-cat-parent>a, .cat-menus .container>ul>li.current-cat-ancestor>a,.cat-menus .children .current-cat a{color:{$skc}}.sitenav > ul > li.menu-item-has-children:hover > a::before{border-top-color:{$skc}}.sitenav ul li:hover > a,.sitenav ul li.active a:hover,.sitenav ul li a:hover{color:{$skc}}.sitenav ul li.current-menu-item>a, .sitenav ul li.current-menu-parent>a, .sitenav ul li.current-category-ancestor>a{color:{$skc};border-bottom-color:{$skc}}.searchform .sinput:focus{border-color:{$skc}}.searchform .sbtn{background-color:{$skc}}.post-like.actived{color:{$skc}}.excerpt time.hot{color:{$skc}}.excerpt-combine footer time.hot{background-color:{$skc}}.pagination ul li.next-page a{background-color:{$skc}}.ias_trigger a{background-color:{$skc}}.article-content h2{border-left:7px solid {$skc}}.article-actions .action-like{background-color:{$skc}}.article-tags a:hover{background-color:{$skc}}.image-navigation a{background-color:{$skc}}.tagslist li .name:hover{background-color:{$skc}}.comt-submit{background:{$skc};border-color:{$skc}}@media (max-width:768px){.sitenav ul li.current-menu-item > a{background-color:{$skc}}.sitenav ul li.current-menu-parent > a{color:{$skc}}.sitenav ul li.current-menu-item>a{color:#fff;}}@media (max-width:544px){.excerpt-combine h2 a:hover{color:{$skc}}.excerpt-combine footer time.hot{color:{$skc}}}";
    }
...
function _posts_related($limit=8){
    ...
    if ( $posttags ) { 
        ...
        while( have_posts() ) { the_post();
            echo '<li><a href="'.get_permalink().'"><span class="thumbnail">'._get_post_thumbnail().'</span>'.get_the_title().'</a></li>';
...
function _notinhome_cats_posts_count(){
    $num = 0;

    foreach (_hui('notinhome') as $key => $value) {
        if( $value ){
            $num += get_category($key)->count;
        }
    }
...
if ( ! function_exists( '_paging' ) ) :
    ...
    function _paging() {
        ...
        if( is_home() && _hui('notinhome') ){
            // 排除不在首页显示的分类文章数
            $out_count = _notinhome_cats_posts_count();
            $published_posts = wp_count_posts()->publish;
            $published_posts = $published_posts - $out_count;
            $posts_per_page = get_option('posts_per_page');
            $max_page = ceil($published_posts / $posts_per_page);
        }
